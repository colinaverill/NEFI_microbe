...
d2 <- readRDS(NEON_dmilti.ddirch_analysis_summary_16S.path)

#Get RSQ and RMSE densities and means for ITS.----
...
#for 16S.----
core.rsq.2 <- d2$validation$val.stat.predictable$core.stat
core.rsq.2 <- data.frame(do.call(rbind, core.rsq.2))
plot.rsq.2 <- d2$validation$val.stat.predictable$plot.stat
plot.rsq.2 <- data.frame(do.call(rbind, plot.rsq.2))
site.rsq.2 <- d2$validation$val.stat.predictable$site.stat
site.rsq.2 <- data.frame(do.call(rbind, site.rsq.2))
core.d.2 <- zero_truncated_density(core.rsq.2$rsq)
plot.d.2 <- zero_truncated_density(plot.rsq.2$rsq)
site.d.2 <- zero_truncated_density(site.rsq.2$rsq)
core.rmse.d.2 <- zero_truncated_density(core.rsq.2$rmse.norm)
plot.rmse.d.2 <- zero_truncated_density(plot.rsq.2$rmse.norm)
site.rmse.d.2 <- zero_truncated_density(site.rsq.2$rmse.norm)
#get calibration values by group.
cal.mu.2 <- d2$calibration$cal.stat.predictable.sum$rsq
val.mu.2 <- d2$validation$site.stat.predictable.sim$rsq
cal.rmse.2 <- d2$calibration$cal.stat.predictable.sum$rmse.norm
val.rmse.2 <- d2$validation$site.stat.predictable.sim$rmse.norm
names(cal.mu.2) <- rownames(d2$calibration$cal.stat.predictable.sum)
...
x <- 1:length(cal.mu.2)
limy <- c(0,max(cal.mu.2)*1.1)
plot(cal.mu.2 ~ x, cex = 2.5, ylim = limy, pch = 16, ylab = NA, xlab = NA, bty='l', xaxt = 'n', yaxs='i', las = 1, lwd = 0)
#arrows(x, lev.mu - lev.se, x1 = x, y1 = lev.mu + lev.se, length=0.00, angle=90, code=3, col = 'black')
lines(x, cal.mu.2, lty = 2)
#lines(x, val.mu.2, lty = 2)
#points(val.mu.2 ~ x, cex = 2.5, pch = 16, col = 'gray')
...
x <- 1:length(cal.rmse.2)
limy <- c(0,max(cal.rmse.2)*1.2)
plot(cal.rmse.2 ~ x, cex = 2.5, ylim = limy, pch = 16, ylab = NA, xlab = NA, bty='l', xaxt = 'n', yaxs='i', las = 1, lwd = 0)
#arrows(x, lev.mu - lev.se, x1 = x, y1 = lev.mu + lev.se, length=0.00, angle=90, code=3, col = 'black')
lines(x, cal.rmse.2, lty = 2)
#lines(x, val.rmse.2, lty = 2)
#points(val.rmse.2 ~ x, cex = 2.5, pch = 16, col = 'gray')
