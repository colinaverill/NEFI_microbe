#variance decomposition for all phylogenetic and functional groups.
rm(list=ls())
source('paths.r')
...
#set output path.----
output.path <- NEON_ddirch_var.decomp_all_groups.path
#load model results.----
#mod 1 is data from maps.
#mod 2 is site-specific data, no maps.
#mod 3 is all covariates.
fg <- readRDS(ted_ITS.prior_fg_JAGSfit)
fg <- fg[[3]] #just the all predictor case.
models <- readRDS(ted_ITS_prior_phylo.group_JAGSfits)
models$function_group <- fg
...
#loop over phylo and function groups, decompose variance.----
super.out <- list()
tic()
for(k in 1:length(models)){
  ...
  super.out[[k]] <- output
  #report.
  cat(k,'of',length(models),'fungal groups decomposed. ');toc()
}
names(super.out) <- names(models)
